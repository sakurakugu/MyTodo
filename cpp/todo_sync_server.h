 ...
 *
 * **重构说明：**
 * 从TodoModel中重构出来的独立同步管理类，实现以下目标：
 * - 分离关注点：将同步逻辑从TodoModel中独立出来
 * - 提高可维护性：同步相关代码集中管理
 * - 增强可测试性：可以独立测试同步功能
 * - 改善代码组织：遵循单一职责原则
 *
...
class TodoSyncServer : public QObject {
  ...
  public:
    ...
    // 属性访问器
    bool isAutoSyncEnabled() const;                    // 获取自动同步是否启用
    void setAutoSyncEnabled(bool enabled);             // 设置自动同步启用状态
