...
class HolidayManager : public QObject {
  ...
  public:
    ...
    bool 初始化();
    DateType 获取日期类型(const QDate &date) const;
    DateType getDateType(const QDate &date) const { return 获取日期类型(date); }
    bool 是否为节假日(const QDate &date) const;
    bool 是否为工作日(const QDate &date) const;
    bool 是否为周末(const QDate &date) const;
    QString 获取节假日名称(const QDate &date) const;
    std::vector<HolidayItem> 获取指定年份的所有节假日(int year) const;
    void 手动刷新指定年份的节假日数据(int year);
    QDate 获取下一个工作日(const QDate &fromDate, int daysToAdd = 1) const;
    QDate 获取下一个节假日(const QDate &fromDate, int daysToAdd = 1) const;
    QDate 获取下一个周末(const QDate &fromDate, int daysToAdd = 1) const;
  ...
  private:
    ...
    QString 获取节假日数据文件路径(int year) const;
    void 从网络获取节假日数据(int year);
    std::vector<HolidayItem> 解析API响应数据(int year, const QJsonObject &response);
    ...
    // 成员变量
    QMap<int, std::vector<HolidayItem>> m_holidayData; ///< 节假日数据缓存 (年份 -> 节假日列表)
    QTimer *m_updateTimer;                             ///< 定时更新定时器
    QMap<int, QDate> m_lastUpdateCheck;                ///< 最后检查更新的日期 (年份 -> 日期)
    NetworkRequest *m_networkRequest;                  ///< 网络请求管理器
    bool m_initialized;                                ///< 是否已初始化
    QString m_holidayDataDir;                     ///< 节假日数据目录

    // 常量
    static const QString HOLIDAY_API_BASE_URL; ///< 节假日API基础URL
    static const int UPDATE_CHECK_INTERVAL_MS; ///< 更新检查间隔（毫秒）
    static const QString HOLIDAY_DATA_DIR;     ///< 节假日数据目录
};