...
/**
 * @brief 设置筛选条件
 * @param filter 筛选条件（如"done"、"todo"、"recycle"、"important"等）
 */
 ...
 * @return 当前重要程度筛选器
 */
bool TodoFilter::currentImportant() const {
 ...
 */
void TodoFilter::setCurrentImportant(bool important) {
...
bool TodoFilter::itemMatchesFilter(const TodoItem *item) const {
    ...
    return checkCategoryMatch(item) && 
           checkStatusMatch(item) && 
           checkImportantMatch(item) && 
           checkDateMatch(item);
}
...
bool TodoFilter::hasActiveFilters() const {
    return !m_currentCategory.isEmpty() ||
...
bool TodoFilter::checkCategoryMatch(const TodoItem *item) const {
    if (m_currentCategory.isEmpty()) {
        return true; // 没有分类筛选，显示所有
    }
    
    return item->category() == m_currentCategory ||
           (m_currentCategory == "uncategorized" && 
            (item->category().isEmpty() || item->category() == "uncategorized"));
}
 ...
 * @return 如果匹配返回true，否则返回false
 */
bool TodoFilter::checkImportantMatch(const TodoItem *item) const {
    // 重要性筛选逻辑：
    ...
    if (m_currentFilter == "important") {
        return (item->important() == m_currentImportant);
    }
