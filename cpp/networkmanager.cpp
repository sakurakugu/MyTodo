...
bool NetworkManager::hasValidAuth() const
{
    ...
    try {
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (shouldRetry(networkError) && request.currentRetry < request.config.maxRetries) {
                ...
                qDebug() << "重试请求:" << request.type << "(" << request.currentRetry << "/" << request.config.maxRetries << ")";
...
void NetworkManager::completeRequest(qint64 requestId, bool success, const QJsonObject &response, const QString &error)
{
    if (!m_pendingRequests.contains(requestId)) {
        return;
    }
    ...
    if (success) {
        ...
        NetworkError networkError = error.contains("超时") ? TimeoutError : 
                                   ...
                                   error.contains("解析") ? ParseError : UnknownError;
...
bool NetworkManager::isDuplicateRequest(RequestType type) const
{
    return m_activeRequests.contains(type);
}

void NetworkManager::addActiveRequest(RequestType type, qint64 requestId)
{
    m_activeRequests[type] = requestId;
}

void NetworkManager::removeActiveRequest(RequestType type)
{
